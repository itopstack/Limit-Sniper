...
def liquidity_check_private_node(token, v1, case):
    ...
    if case == 2 or case == 3:
        ...
        printt_debug("custom_base_price_in_dollars:", custom_base_price_in_dollars)
...
def buy(token_dict, inToken, outToken, pwd):
    ...
    if balance > Decimal(amount):
        ...
        if multiplebuys.lower() == 'true':
            ...
            while True:
                if buynumber < amount_of_buys:
                    ...
                    if token_dict['KIND_OF_SWAP'] == 'tokens':
                        make_the_buy_exact_tokens(token_dict, inToken, outToken, buynumber, pwd, gaslimit, routing, custom, slippage, CONTRACT_DECIMALS, balance, nonce)
                    else:
                        make_the_buy(inToken, outToken, buynumber, pwd, amount, int(token_dict['_GAS_TO_USE']), gaslimit, gaspriority, routing, custom, slippage, CONTRACT_DECIMALS, nonce)
                    buynumber += 1
                else:
            ...
            if token_dict['KIND_OF_SWAP'] == 'tokens':
                tx_hash = make_the_buy_exact_tokens(token_dict, inToken, outToken, buynumber, pwd, gaslimit, routing, custom, slippage, CONTRACT_DECIMALS, balance, nonce)
            else:
                tx_hash = make_the_buy(inToken, outToken, buynumber, pwd, amount, int(token_dict['_GAS_TO_USE']), gaslimit, gaspriority, routing, custom, slippage, CONTRACT_DECIMALS, nonce)
            ...
            for token in tokens:
                ...
                if token['ENABLED'] == 'true':
                    ...
                    if token['_TOKEN_BALANCE'] < float(token['MAXTOKENS']) and token['_BUY_IS_MADE'] == False:
                        ...
                        if buy_TxHash != False:
                            txbuyresult = wait_for_tx(token, buy_TxHash, token['ADDRESS'])
                            ...
                            if txbuyresult != 1:
                                ...
                                printt_ok("")
                                buy_tx_detail = client.eth.get_transaction(buy_TxHash)
