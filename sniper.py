...
parser.add_argument("--reject_already_existing_liquidity", action='store_true',
                    help="If liquidity is found on the first check, reject that pair.")
parser.add_argument("-s", "--settings", type=str,
                    help="Specify the file to user for settings (default: settings.json)", default="./settings.json")
...
parser.add_argument("-pc", "--password_on_change", action='store_true',
                    help="Ask user password again if you change tokens.json")
parser.add_argument("-sm", "--slow_mode", action='store_true',
                    help="Bot will only check price 2 times/s. Use it if you're on a RPC with rate limit")
...
def printt(*print_args, write_to_log=False):
    ...
    if bot_settings['_NEED_NEW_LINE'] == True:
        print()
...
def printt_err(*print_args, write_to_log=True):
    ...
    if bot_settings['_NEED_NEW_LINE'] == True:
        print()
    print(timestamp(), " ", style.RED, ' '.join(
        map(str, print_args)), style.RESET, sep="")
...
def printt_warn(*print_args, write_to_log=False):
    ...
    if bot_settings['_NEED_NEW_LINE'] == True:
        print()
    print(timestamp(), " ", style.YELLOW, ' '.join(
        map(str, print_args)), style.RESET, sep="")
...
def printt_ok(*print_args, write_to_log=False):
    ...
    if bot_settings['_NEED_NEW_LINE'] == True:
        print()
    print(timestamp(), " ", style.GREEN, ' '.join(
        map(str, print_args)), style.RESET, sep="")
...
def printt_info(*print_args, write_to_log=False):
    ...
    if bot_settings['_NEED_NEW_LINE'] == True:
        print()
    print(timestamp(), " ", style.INFO, ' '.join(
        map(str, print_args)), style.RESET, sep="")
...
def printt_debug(*print_args, write_to_log=False):
    ...
    if command_line_args.debug == True:
        print(timestamp(), " ", style.DEBUG, ' '.join(
            map(str, print_args)), style.RESET, sep="")
...
def printt_sell_price(token_dict, token_price):
    ...
    printt_debug("token_dict['_TRADING_IS_ON'] 266:",
                 token_dict['_TRADING_IS_ON'], "for token:", token_dict['SYMBOL'])
    printt_debug("_PREVIOUS_QUOTE :",
                 token_dict['_PREVIOUS_QUOTE'], "for token:", token_dict['SYMBOL'])

    if token_dict['USECUSTOMBASEPAIR'] == 'false':
        price_message = token_dict['_PAIR_SYMBOL'] + " Price: " + \
            "{0:.24f}".format(token_price) + " " + base_symbol

    else:
        price_message = token_dict['_PAIR_SYMBOL'] + " Price: " + \
            "{0:.24f}".format(token_price) + " " + token_dict['BASESYMBOL']

    price_message = price_message + " - Sell:" + \
        str(token_dict['_CALCULATED_SELLPRICEINBASE']) + " Stop:" + \
        str(token_dict['_CALCULATED_STOPLOSSPRICEINBASE'])
    # price_message = price_message + " ATH:" + "{0:.24f}".format(token_dict['_ALL_TIME_HIGH']) + " ATL:" + "{0:.24f}".format(token_dict['_ALL_TIME_LOW'])

    if token_dict['USECUSTOMBASEPAIR'] == 'false':
        price_message = price_message + " - Token balance: " + str("{0:.4f}".format(token_dict['_TOKEN_BALANCE'])) + " (= " + "{0:.2f}".format(
            float(token_price) * float(token_dict['_BASE_PRICE']) * float(token_dict['_TOKEN_BALANCE'])) + " $)"
    else:
        price_message = price_message + " - Token balance: " + str("{0:.4f}".format(token_dict['_TOKEN_BALANCE'])) + " (= " + "{0:.2f}".format(
            float(token_price) * float(token_dict['_TOKEN_BALANCE'])) + " " + token_dict['BASESYMBOL'] + ")"

    # Display a message if MAXTOKENS is reached
    if token_dict['_REACHED_MAX_TOKENS'] == True:
        price_message = price_message + \
            '\033[31m' + " - MAXTOKENS reached" + '\033[0m'
...
def load_settings_file(settings_path, load_message=True):
    ...
    for value in program_defined_values:
        if value not in bot_settings:
            bot_settings[value] = program_defined_values[value]
    ...
    for default_true in default_true_settings:
        if default_true not in settings:
            print(timestamp(), default_true,
                  "not found in settings.json, settings a default value of true.")
    ...
    for required_setting in required_user_settings:
        if required_setting not in settings:
            print(timestamp(), "ERROR:", required_setting,
...
def apprise_notification(token, parameter):
    ...
    try:
        if parameter == 'buy_success':
            message = "Your " + \
                token['SYMBOL'] + " buy Tx is confirmed. Price : " + \
                str("{:.10f}".format(token['_QUOTE']))
            title = "BUY Success"
            ...
            message = "Your " + \
                token['SYMBOL'] + " sell Tx is confirmed. Price : " + \
                str("{:.10f}".format(token['_QUOTE']))
            title = "SELL Success"
...
def load_tokens_file(tokens_path, load_message=True):
    ...
    # _BUY_IS_MADE          - defines if BUY order has been made --> switching to SELL mode
    ...
    for token in tokens:
        ...
        for required_key in required_user_settings:
            if required_key not in token:
                printt_err(
                    required_key, "not found in configuration file in configuration for to token", token['SYMBOL'])
                printt_err(
                    "Be careful, sometimes new parameter are added : please check default tokens.json file")
                sleep(20)
                exit(-1)
        ...
        for default_true in default_true_settings:
            if default_true not in token:
                printt_v(default_true, "not found in configuration file in configuration for to token",
                         token['SYMBOL'], "setting a default value of true")
        ...
        for default_key in default_value_settings:
            if default_key not in token:
                printt_v(default_key, "not found in configuration file in configuration for to token",
                         token['SYMBOL'], "setting a value of", default_value_settings[default_key])
        ...
        if token['WATCH_STABLES_PAIRS'] == 'true' and token['USECUSTOMBASEPAIR'] == 'false':
            ...
            printt("")
            printt_warn("Ignoring WATCH_STABLES_PAIRS", "for",
                        token['SYMBOL'], ": WATCH_STABLES_PAIRS = true and USECUSTOMBASEPAIR = true is unsupported.")
            printt("")

        if token['USECUSTOMBASEPAIR'] == 'true' and token['LIQUIDITYINNATIVETOKEN'] == 'false':
            token['_PAIR_SYMBOL'] = token['SYMBOL'] + '/' + token['BASESYMBOL']
        else:
            token['_PAIR_SYMBOL'] = token['SYMBOL'] + \
                '/' + token['_EXCHANGE_BASE_SYMBOL']
...
def reload_tokens_file(tokens_path, load_message=True):
    ...
    if load_message == True:
        ...
        printt("Reloading tokens from", tokens_path,
               '\033[31m', "- do NOT change token SYMBOL in real time", '\033[0m', write_to_log=True)
        printt("")
    ...
    for token in tokens:
        ...
        token.update({
            ...
            '_LIQUIDITY_DECIMALS': _TOKENS_saved[token['SYMBOL']]['_WETH_DECIMALS'],
...
def token_list_report(tokens, all_pairs=False):
    ...
    if all_pairs == False:
        printt("Quantity of tokens attempting to trade:",
               tokens_trading, "(", token_list, ")")
    else:
        printt("Quantity of tokens attempting to trade:",
...
if settings['EXCHANGE'] == 'pancakeswap':
    ...
    if my_provider[0].lower() == 'h':
        ...
        client = Web3(Web3.WebsocketProvider(
            my_provider, websocket_timeout=360, websocket_kwargs={"max_size": 650000000}))
    else:
    ...
    if settings['EXCHANGEVERSION'] == "1":
        routerAddress = Web3.toChecksumAddress(
    ...
    factoryContract = client.eth.contract(
        address=factoryAddress, abi=factoryAbi)
...
if settings['EXCHANGE'].lower() == 'pancakeswaptestnet':
    ...
    print(timestamp(), "Binance Smart Chain testnet Connected =",
          client.isConnected())
    print(timestamp(), "Loading Smart Contracts...")
...
if settings["EXCHANGE"] == 'pangolin':
    ...
    routerContract = client.eth.contract(
        address=routerAddress, abi=pangolinAbi)
    factoryContract = client.eth.contract(
        address=factoryAddress, abi=factoryAbi)
    weth = Web3.toChecksumAddress("0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7")
    base_symbol = "AVAX"
...
if settings['EXCHANGE'] == 'pinkswap':
    ...
    factoryAddress = Web3.toChecksumAddress(
        "0x7d2ce25c28334e40f37b2a068ec8d5a59f11ea54")
...
if settings['EXCHANGE'] == 'bakeryswap':
    ...
    routerContract = client.eth.contract(
        address=routerAddress, abi=bakeryRouter)
    factoryContract = client.eth.contract(
        address=factoryAddress, abi=factoryAbi)
...
if settings['EXCHANGE'] == 'apeswap':
    ...
    factoryAddress = Web3.toChecksumAddress(
        "0xAE46cBBCDFBa3bE0F02F463Ec5486eBB4e2e65Ae")
    routerContract = client.eth.contract(address=routerAddress, abi=routerAbi)
    factoryContract = client.eth.contract(
        address=factoryAddress, abi=factoryAbi)
    ...
    settings['_STABLE_BASES'] = {'USD ': {
        'address': '0x0039f574ee5cc39bdd162e9a88e3eb1f111baf48', 'multiplier': 0}}
    ...
    factoryAddress = Web3.toChecksumAddress(
        "0xC0fFeE00000e1439651C6aD025ea2A71ED7F3Eab")
    ...
    routerContract = client.eth.contract(
        address=routerAddress, abi=protofirouter)
    factoryContract = client.eth.contract(
        address=factoryAddress, abi=factoryAbi)
    weth = Web3.toChecksumAddress("0x21be370d5312f44cb42ce377bc9b8a0cef1a4c83")
    base_symbol = "FTM"
    ...
    settings['_STABLE_BASES'] = {'USDC': {
        'address': '0xc21223249ca28397b4b6541dffaecc539bff0c59', 'multiplier': 0}}
...
def get_password():
    ...
    if 'ENCRYPTPRIVATEKEYS' not in settings:
        ...
        while response != "y" and response != "n":
            ...
            response = input(
                "You will need to input this password each time LimitSwap is executed (y/n): ")
...
def save_settings(settings, pwd):
    if len(pwd) > 0:
        ...
        encrypted_settings['LIMITWALLETPRIVATEKEY'] = 'aes:' + \
            cryptocode.encrypt(settings['LIMITWALLETPRIVATEKEY'], pwd)
        encrypted_settings['PRIVATEKEY'] = 'aes:' + \
            cryptocode.encrypt(settings['PRIVATEKEY'], pwd)
        if settings['PRIVATEKEY2'] != 'null':
            encrypted_settings['PRIVATEKEY2'] = 'aes:' + \
                cryptocode.encrypt(settings['PRIVATEKEY2'], pwd)
        if settings['PRIVATEKEY3'] != 'null':
            encrypted_settings['PRIVATEKEY3'] = 'aes:' + \
                cryptocode.encrypt(settings['PRIVATEKEY3'], pwd)
        if settings['PRIVATEKEY4'] != 'null':
            encrypted_settings['PRIVATEKEY4'] = 'aes:' + \
                cryptocode.encrypt(settings['PRIVATEKEY4'], pwd)
        if settings['PRIVATEKEY5'] != 'null':
            encrypted_settings['PRIVATEKEY5'] = 'aes:' + \
                cryptocode.encrypt(settings['PRIVATEKEY5'], pwd)
...
def build_extended_base_configuration(token_dict):
    ...
    for stable_token in settings['_STABLE_BASES']:
        ...
        if not re.search('^(\d+\.){0,1}\d+(x|X|%)$', str(token_dict['SELLPRICEINBASE'])):
            new_token['SELLPRICEINBASE'] = float(
                token_dict['SELLPRICEINBASE']) * float(settings['_STABLE_BASES'][stable_token]['multiplier'])
        if not re.search('^(\d+\.){0,1}\d+(x|X|%)$', str(token_dict['STOPLOSSPRICEINBASE'])):
            new_token['STOPLOSSPRICEINBASE'] = float(
                token_dict['STOPLOSSPRICEINBASE']) * float(settings['_STABLE_BASES'][stable_token]['multiplier'])
...
def parse_wallet_settings(settings, pwd):
    ...
    if " " in settings['LIMITWALLETADDRESS'] or settings['LIMITWALLETADDRESS'] == "":
        settings_changed = True
        settings['LIMITWALLETADDRESS'] = input(
            "Please provide the wallet address where you have your LIMIT: ")
    ...
    # If the limit wallet private key is already set and encrypted, decrypt it
        ...
        settings['LIMITWALLETPRIVATEKEY'] = settings['LIMITWALLETPRIVATEKEY'].replace(
            'aes:', "", 1)
        settings['LIMITWALLETPRIVATEKEY'] = cryptocode.decrypt(
            settings['LIMITWALLETPRIVATEKEY'], pwd)
    ...
    if " " in settings['WALLETADDRESS'] or settings['WALLETADDRESS'] == "":
        settings_changed = True
        settings['WALLETADDRESS'] = input(
            "Please provide the wallet address for your trading wallet: ")

    # Check for trading wallet private key
    if " " in settings['PRIVATEKEY'] or settings['PRIVATEKEY'] == "":
        settings_changed = True
        settings['PRIVATEKEY'] = input(
            "Please provide the private key for the wallet you want to trade with: ")
    ...
    # If the trading wallet private key is already set and encrypted, decrypt it
        ...
        settings['PRIVATEKEY'] = cryptocode.decrypt(
            settings['PRIVATEKEY'], pwd)
    ...
    if settings['WALLETADDRESS2'] == 'no_utility' or settings['PRIVATEKEY2'].startswith('aes:'):
        ...
        if decision == "y":
            ...
            if " " in settings['WALLETADDRESS2'] or settings['WALLETADDRESS2'] == "null":
                settings_changed = True
                settings['WALLETADDRESS2'] = input(
                    "Please provide the 2nd trading wallet address: ")

            # Check for trading wallet private key
            if " " in settings['PRIVATEKEY2'] or settings['PRIVATEKEY2'] == "null":
                settings_changed = True
                settings['PRIVATEKEY2'] = input(
                    "Please provide the 2nd private key for the 2nd trading wallet: ")
            stoptheprocess = 0
        else:
    ...
    if stoptheprocess != 1:
        ...
        if decision == "y":
            ...
            if " " in settings['WALLETADDRESS3'] or settings['WALLETADDRESS3'] == "null":
                settings_changed = True
                settings['WALLETADDRESS3'] = input(
                    "Please provide the 3rd trading wallet address: ")

            # Check for trading wallet private key
            if " " in settings['PRIVATEKEY3'] or settings['PRIVATEKEY3'] == "null":
                settings_changed = True
                settings['PRIVATEKEY3'] = input(
                    "Please provide the 3rd private key for the 3rd trading wallet: ")
            stoptheprocess = 0
        else:
            stoptheprocess = 1
    ...
    if stoptheprocess != 1:
        ...
        if decision == "y":
            ...
            if " " in settings['WALLETADDRESS4'] or settings['WALLETADDRESS4'] == "null":
                settings_changed = True
                settings['WALLETADDRESS4'] = input(
                    "Please provide the 4th trading wallet address: ")

            # Check for trading wallet private key
            if " " in settings['PRIVATEKEY4'] or settings['PRIVATEKEY4'] == "null":
                settings_changed = True
                settings['PRIVATEKEY4'] = input(
                    "Please provide the 4th private key for the 4th trading wallet: ")
            stoptheprocess = 0
        else:
            stoptheprocess = 1
    ...
    if stoptheprocess != 1:
        ...
        if decision == "y":
            ...
            if " " in settings['WALLETADDRESS5'] or settings['WALLETADDRESS5'] == "null":
                settings_changed = True
                settings['WALLETADDRESS5'] = input(
                    "Please provide the 5th trading wallet address: ")

            # Check for trading wallet private key
            if " " in settings['PRIVATEKEY5'] or settings['PRIVATEKEY5'] == "null":
                settings_changed = True
                settings['PRIVATEKEY5'] = input(
                    "Please provide the 5th private key for the 5th trading wallet: ")
...
def decimals(address):
    ...
    try:
        balanceContract = client.eth.contract(
            address=Web3.toChecksumAddress(address), abi=standardAbi)
...
def auth():
    ...
    print(timestamp(), "Connected to ETH to check your LIMIT tokens =",
          client2.isConnected())
    ...
    try:
        ...
        printt_err(
            "There is a problem with your private key: please check if it's correct. Don't enter your seed phrase !")
...
def approve(address, amount):
    ...
    eth_balance = Web3.fromWei(client.eth.getBalance(
        settings['WALLETADDRESS']), 'ether')
    ...
    if eth_balance > minimumbalance:
        ...
        if settings['EXCHANGE'] == 'uniswaptestnet':
            ...
            gas = (((client.eth.gasPrice) / 1000000000)) + \
                ((client.eth.gasPrice) / 1000000000) * (int(200) / 100)
            printt("Current Gas Price =", gas)
        else:
            gas = (((client.eth.gasPrice) / 1000000000)) + \
                ((client.eth.gasPrice) / 1000000000) * (int(20) / 100)
            printt("Current Gas Price = ", gas)
        ...
        signed_txn = client.eth.account.signTransaction(
            transaction, private_key=settings['PRIVATEKEY'])

        try:
            return client.eth.sendRawTransaction(signed_txn.rawTransaction)
        finally:
            printt("Transaction Hash = ", Web3.toHex(
                client.keccak(signed_txn.rawTransaction)), write_to_log=True)
...
def check_approval(token, address, allowance_to_compare_with, condition):
    ...
    contract = client.eth.contract(
        address=Web3.toChecksumAddress(address), abi=standardAbi)
    actual_allowance = contract.functions.allowance(
        Web3.toChecksumAddress(settings['WALLETADDRESS']), routerAddress).call()
        ...
        if settings["EXCHANGE"] == 'quickswap':
            ...
            if condition == 'base_approve':
                printt_info(
                    "---------------------------------------------------------------------------------------------")
                printt_info(
                    "Your base token is not approved --> LimitSwap will now APPROVE it, or it won't be able to buy")
                printt_info(
                    "---------------------------------------------------------------------------------------------")
                ...
                printt_info(
                    "-----------------------------------------------------------------------------")
                printt_info(
                    "You have selected PREAPPROVE = true --> LimitSwap will now APPROVE this token")
                printt_info(
                    "-----------------------------------------------------------------------------")
                ...
                printt_info(
                    "----------------------------------------------------------------------------------")
                printt_info(
                    "You have failed to sell tokens --> LimitSwap will chack if it needs to be APPROVED")
                printt_info(
                    "----------------------------------------------------------------------------------")
            else:
                printt_info("-------------------------------------")
            ...
            printt_ok("---------------------------------------------------------")
            printt_ok(
                "  Token is now approved : LimitSwap can sell this token", write_to_log=True)
            printt_ok("---------------------------------------------------------")
...
def check_bnb_balance():
    ...
    printt("Current Wallet Balance is :", Web3.fromWei(
        balance, 'ether'), base_symbol, write_to_log=True)
    return balance
...
def check_balance(address, symbol='UNKNOWN_TOKEN', display_quantity=True):
    ...
    balance = balanceContract.functions.balanceOf(
        settings['WALLETADDRESS']).call()
    if display_quantity == True:
        printt("Current Wallet Balance is: ", str(
            balance / DECIMALS), symbol, write_to_log=True)
    else:
...
def sync(inToken, outToken):
    ...
    syncContract = client.eth.contract(
        address=Web3.toChecksumAddress(pair), abi=lpAbi)
...
def check_rugdoc_api(token):
    ...
    if token['ADDRESS'] not in rugdoc_accepted_tokens:
        ...
        while token['_RUGDOC_DECISION'] != "y" and token['_RUGDOC_DECISION'] != "n":
            ...
            token['_RUGDOC_DECISION'] = input(
                "                           Would you like to snipe this token? (y/n): ")
...
def pre_calculate_custom_bases_pairs(base_symbol):
    ...
    if base_symbol == "BNB" or base_symbol == "BNB ":
        ...
        USDT = calculate_custom_base_price_big_ttl(Web3.toChecksumAddress(
            '0x55d398326f99059ff775485246999027b3197955'), 1000000000000000000, 1000000000000000000)
        printt_debug("USDT price :", 1/USDT, "\n")

        BUSD = calculate_custom_base_price_big_ttl(Web3.toChecksumAddress(
            '0xe9e7cea3dedca5984780bafc599bd69add087d56'), 1000000000000000000, 1000000000000000000)
        printt_debug("BUSD price :", 1/BUSD, "\n")

        USDC = calculate_custom_base_price_big_ttl(Web3.toChecksumAddress(
            '0x8ac76a51cc950d9822d68b83fe1ad97b32cd580d'), 1000000000000000000, 1000000000000000000)
        printt_debug("USDC price :", 1/USDC, "\n")

    elif base_symbol == "BNBt":
        printt_debug("pre_calculate_custom_bases_pairs for BNBt")
        BNBt = calculate_custom_base_price_big_ttl(Web3.toChecksumAddress(
            '0x78867bbeef44f2326bf8ddd1941a4439382ef2a7'), 1000000000000000000, 1000000000000000000)
        ...
        USDT = calculate_custom_base_price_big_ttl(Web3.toChecksumAddress(
            '0xdac17f958d2ee523a2206206994597c13d831ec7'), 1000000, 1000000000000000000)
        printt_debug("USDT price :", 1/USDT, "\n")

        USDC = calculate_custom_base_price_big_ttl(Web3.toChecksumAddress(
            '0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48'), 1000000, 1000000000000000000)
        printt_debug("USDC price :", 1/USDC, "\n")

    elif base_symbol == "AVAX":
        printt_debug("pre_calculate_custom_bases_pairs for AVAX")

        MIM = calculate_custom_base_price_big_ttl(Web3.toChecksumAddress(
            '0x130966628846bfd36ff31a822705796e8cb8c18d'), 1000000000000000000, 1000000000000000000)
        printt_debug("MIM price :", 1/MIM, "\n")

        USDC = calculate_custom_base_price_big_ttl(Web3.toChecksumAddress(
            '0xA7D7079b0FEaD91F3e65f86E8915Cb59c1a4C664'), 1000000, 1000000000000000000)
        printt_debug("USDC price :", 1/USDC, "\n")

        USDCb = calculate_custom_base_price_big_ttl(Web3.toChecksumAddress(
            '0xb97ef9ef8734c71904d8002f8b6bc66dd9c48a6e'), 1000000, 1000000000000000000)
        printt_debug("USDCb price :", 1/USDCb, "\n")

        USDT = calculate_custom_base_price_big_ttl(Web3.toChecksumAddress(
            '0xc7198437980c041c805a1edcba50c1ce5db95118'), 1000000, 1000000000000000000)
        printt_debug("USDT price :", 1/USDT, "\n")
        ...
        USDC = calculate_custom_base_price_big_ttl(Web3.toChecksumAddress(
            '0x04068da6c83afcfa0e13ba15a6696662335d5b75'), 1000000, 1000000000000000000)
        printt_debug("USDC price :", 1/USDC, "\n")

        USDT = calculate_custom_base_price_big_ttl(Web3.toChecksumAddress(
            '0x049d68029688eabf473097a2fc38ef61633a3c7a'), 1000000, 1000000000000000000)
        printt_debug("USDT price :", 1/USDT, "\n")
        ...
        USDT = calculate_custom_base_price_big_ttl(Web3.toChecksumAddress(
            '0xc2132d05d31c914a87c6611c10748aeb04b58e8f'), 1000000, 1000000000000000000)
        printt_debug("USDT price :", 1/USDT, "\n")

        USDC = calculate_custom_base_price_big_ttl(Web3.toChecksumAddress(
            '0x2791bca1f2de4661ed88a30c99a7a9449aa84174'), 1000000, 1000000000000000000)
        printt_debug("USDC price :", 1/USDC, "\n")
...
def scan_mempool_public_node(token):
    ...
    printt("")
    printt_warn(
        "Do not make any transaction with this wallet before bot buys, because Nonce is pre-calculated")
    printt("")
    printt("--------------------------------------------------------")
    ...
    while buyToken == False:
        ...
        printt("Scanning Mempool & Waiting for New Liquidity Add Event..... Current Block: ",
        ...
        for pending in pending_transactions:
            ...
            if pending['to'] == to_address:
                ...
                try:
                    ...
                    logging.exception(ve)
                    printt_err(
                        "An error occured. Please check your logs and report it to the team.")
                    break
                ...
                if str(decoded[0]) == '<Function addLiquidityETH(address,uint256,uint256,uint256,address,uint256)>' \
                    ...
                    try:
                        if filter_contract['token'] == Web3.toChecksumAddress(token['ADDRESS']):
                            ...
                            token['LIQUIDITYINNATIVETOKEN'] = 'true'
                            token['USECUSTOMBASEPAIR'] = 'false'
                            token_check = True
                        else:
                    ...
                    # Some liquidity functions use tokenA and tokenB instead
                        ...
                        if filter_contract['tokenA'] == Web3.toChecksumAddress(token['ADDRESS']):
                            token['LIQUIDITYINNATIVETOKEN'] = 'false'
                            token['USECUSTOMBASEPAIR'] = 'true'
                            token['_OUT_TOKEN'] = filter_contract['tokenB']
                            token['BASEADDRESS'] = filter_contract['tokenB']
                            token['BASESYMBOL'] = base_symbol
                            token_check = True
                        elif filter_contract['tokenB'] == Web3.toChecksumAddress(token['ADDRESS']):
                            token['LIQUIDITYINNATIVETOKEN'] = 'false'
                            token['USECUSTOMBASEPAIR'] = 'true'
                            token['_OUT_TOKEN'] = filter_contract['tokenA']
                            token['BASEADDRESS'] = filter_contract['tokenA']
                            token['BASESYMBOL'] = base_symbol
                            token_check = True
                    ...
                    if token_check:
                        ...
                        printt_debug(
                            "pending['gasPrice']:", pending['gasPrice'])
                        token['_GAS_IS_CALCULATED'] = True
                        token['_GAS_TO_USE'] = int(
                            pending['gasPrice']) / 1000000000
                        ...
                        printt_ok("")
                        printt("- Block number:",
                               pending_block['number'], write_to_log=True)
                        printt("- Function:",
                               str(decoded[0]), write_to_log=True)
                        printt("- TxHash:", AddLiquidityTxHash,
                               write_to_log=True)
                        printt("- GAS will be the same as liquidity adding event for your BUY Tx: GAS=",
                               token['_GAS_TO_USE'], write_to_log=True)
                        printt_ok("")
                        printt_ok(
                            "--------------------------------------------------------")
                        ...
                        if token["MINIMUM_LIQUIDITY_IN_DOLLARS"] != 0:
                            if token['LIQUIDITYINNATIVETOKEN'] == 'true':
                                ...
                                liquidity_amount = pending['value'] / \
                                    token['_LIQUIDITY_DECIMALS']

                            elif token['LIQUIDITYINNATIVETOKEN'] == 'false' and token['USECUSTOMBASEPAIR'] == 'true':
                                printt_debug(
                                    "token['ADDRESS'].lower()", token['BASEADDRESS'].lower())
                                printt_debug(
                                    "decoded['tokenA']", filter_contract['tokenA'].lower())
                                printt_debug(
                                    "decoded['tokenB']", filter_contract['tokenB'].lower())
                                printt_debug(
                                    "decoded['amountBDesired']", filter_contract['amountBDesired'])
                                printt_debug(
                                    "decoded['amountBDesired']", filter_contract['amountBDesired'])
                                if token['BASEADDRESS'].lower() == filter_contract['tokenA'].lower():
                                    printt_debug(
                                        "the token we're trying to snipe is tokenA --> let's find amount in amountADesired")
                                    liquidity_amount = filter_contract['amountADesired'] / \
                                        token['_LIQUIDITY_DECIMALS']
                                else:
                                    printt_debug(
                                        "the token we're trying to snipe is tokenB --> let's find amount in amountBDesired")
                                    liquidity_amount = filter_contract['amountBDesired'] / \
                                        token['_LIQUIDITY_DECIMALS']

                            liquidity_result = check_liquidity_amount_mempool(
                                token, liquidity_amount)
                            if liquidity_result != 0:
                                ...
                                while response != "y" and response != "n":
                                    ...
                                    response = input(
                                        "                 Would you like to restart the bot and scan mempool again? (y/n): ")
...
def scan_mempool_private_node(token, methodid):
    ...
    # Determine nonce
    nonce = client.eth.getTransactionCount(
    ...
    printt("")
    printt_warn(
        "Don't worry if nothing appears : it's normal! Do not close the bot")
    printt("")
    printt("--------------------------------------------------------")
                    ...
                    if v1['input'][:10] in methodid:
                        ...
                        try:
                            if input_decoded[1]['token'] == tokenAddress:
                                ...
                                if token['BUYAFTER_XXX_SECONDS'] != 0:
                                    printt_info(
                                        "Bot will wait", token['BUYAFTER_XXX_SECONDS'], " seconds before buy, as you entered in BUYAFTER_XXX_SECONDS parameter")
                                ...
                                signed_tx_condition_1 = client.eth.account.signTransaction(
                                    tx_condition_1, private_key=settings['PRIVATEKEY'])

                                # Send transaction (it has already been created before)
                                buy_tx_hash = client.eth.sendRawTransaction(
                                    signed_tx_condition_1.rawTransaction)
                            ...
                            if input_decoded[1]['tokenA'] == tokenAddress:
                                ...
                                tx_condition_2 = routerContract.functions.swapExactETHForTokens(
                                    ...
                                    [weth, Web3.toChecksumAddress(
                                        input_decoded[1]['tokenB']), tokenAddress],
                                    walletused,
                                    int(time() + + 60)
                                ).buildTransaction({
                                    'gasPrice': gasprice,
                                ...
                                # Sign the transaction
                                signed_tx_condition_2 = client.eth.account.signTransaction(
                                    tx_condition_2, private_key=settings['PRIVATEKEY'])

                                # Send transaction
                                buy_tx_hash = client.eth.sendRawTransaction(
                                    signed_tx_condition_2.rawTransaction)

                                txMade = True

                            elif input_decoded[1]['tokenB'] == tokenAddress:
                                printt_debug("buy condition 3",
                                             write_to_log=True)
                                ...
                                tx_condition_3 = routerContract.functions.swapExactETHForTokens(
                                    0,
                                    [weth, Web3.toChecksumAddress(
                                        input_decoded[1]['tokenA']), tokenAddress],
                                    walletused,
                                    int(time() + + 60)
                                ).buildTransaction({
                                    'gasPrice': gasprice,
                                ...
                                # Sign the transaction
                                signed_tx_condition_3 = client.eth.account.signTransaction(
                                    tx_condition_3, private_key=settings['PRIVATEKEY'])

                                # Send transaction
                                buy_tx_hash = client.eth.sendRawTransaction(
                                    signed_tx_condition_3.rawTransaction)
                        ...
                        if txMade:
                            ...
                            printt_ok(
                                "--------------------------------------------------------")
                            printt_ok(
                                "Bot detected a AddLiquidity Event:", write_to_log=True)
                            printt("")
                            printt("- MethodID:", v1['input'][:10])
                            printt("- from:", v1['from'])
                            printt("- AddLiquidity TxHash:",
                                   AddLiquidityTxHash, write_to_log=True)
                            printt("")
                            printt("- And made a BUY order :",
                                   Web3.toHex(buy_tx_hash), write_to_log=True)
                            if token["GAS"] == 'same_as_tx':
                                printt("- With same Gas as AddLiquidity Tx:",
                                       int(v1['gasPrice'], 16) / 1000000000)
                            else:
...
def scan_mempool_pinksale_private_node(token):
    ...
    tokenPresaleAddressCheckSum = Web3.toChecksumAddress(
        token['PINKSALE_PRESALE_ADDRESS'])
                ...
                for k1, v1 in v.items():
                    ...
                    if v1['input'] == '0x4bb278f3':
                        ...
                        if v1['to'].lower() == tokenPresaleAddress:
                            #  it's the token you've put in your tokens.json --> token detected, let's snipe !
                            printt_debug("buy condition pinksale",
                                         write_to_log=True)
                            ...
                            # Sign the transaction
                            signed_tx_condition_pinksale = client.eth.account.signTransaction(
                                tx_condition_pinksale, private_key=settings['PRIVATEKEY'])

                            # Send transaction (it has already been created before)
                            buy_tx_hash = client.eth.sendRawTransaction(
                                signed_tx_condition_pinksale.rawTransaction)
                        ...
                        if txMade:
                            ...
                            printt_ok(
                                "--------------------------------------------------------")
                            printt_ok(
                                "Bot detected a finalize() event:", write_to_log=True)
                            printt("")
                            printt("- MethodID:", v1['input'][:10])
                            printt("- from:", v1['from'])
                            printt("- AddLiquidity TxHash:",
...
def scan_mempool_private_node_work_in_progress(token, methodid):
    printt_debug(
        "ENTER scan_mempool_private_node_work_in_progress", write_to_log=True)
...
def liquidity_check_private_node(token, v1, case):
    printt_debug("ENTER liquidity_check_private_node")

    # Function: liquidity_check_private_node
    # ----------------------------
    ...
    # Let's calculate Liquidity amount added. 2 cases :
    #   1/ If liquidity is in native token (BNB...)      the amount is situated in v1['value']
    #   2/ If liquidity is NOT in native token (BUSD...) the amount is situated in v1['amountADesired'] or v1['amountBDesired']

    if case == 1:
        # Easiest case
        printt_debug("liquidity_check_private_node case token")
        liquidity_amount = int(v1['value'], 16) / token['_LIQUIDITY_DECIMALS']
        liquidity_amount_in_dollars = float(
            liquidity_amount) * float(token['_BASE_PRICE'])
    elif case == 2:
        printt_debug("liquidity_check_private_node case token A")
        input_decoded = routerContract.decode_function_input(v1['input'])
        liquidity_amount = input_decoded[1]['amountBDesired'] / \
            token['_LIQUIDITY_DECIMALS']
        ...
        input_decoded = routerContract.decode_function_input(v1['input'])
        liquidity_amount = input_decoded[1]['amountADesired'] / \
            token['_LIQUIDITY_DECIMALS']
        printt_debug("liquidity_amount:", liquidity_amount)
    ...
    if case == 2 or case == 3:
        ...
        custom_base_price_in_base = calculate_custom_base_price_big_ttl(
            outToken, token['_BASE_DECIMALS'], token['_WETH_DECIMALS'])
        printt_debug("custom_base_price_in_base:", custom_base_price_in_base)

        # 2/ convert this Custom Base token price in $
        custom_base_price_in_dollars = float(
            custom_base_price_in_base) * float(token['_BASE_PRICE'])
        printt_debug("custom_base_price_in_dollars:",
                     custom_base_price_in_dollars)
        liquidity_amount_in_dollars = float(
            liquidity_amount) * float(custom_base_price_in_dollars)
    ...
    if float(token['MINIMUM_LIQUIDITY_IN_DOLLARS']) <= float(liquidity_amount_in_dollars):
        ...
        printt_ok("------------------------------------------------",
                  write_to_log=True)
        printt_ok(
            "WE FOUND SOMETHING IN MEMPOOL - let's check liquidity", write_to_log=True)
        printt_ok("Tx:", v1['hash'], write_to_log=True)
        printt_ok("")
        printt_ok("ENOUGH LIQUIDITY", write_to_log=True)
        printt_ok("", write_to_log=True)
        printt_ok("- You have set MINIMUM_LIQUIDITY_IN_DOLLARS  =",
                  token['MINIMUM_LIQUIDITY_IN_DOLLARS'], "$", write_to_log=True)
        printt_ok("- Liquidity added for", token['SYMBOL'], "=", "{:.14g}".format(
            liquidity_amount_in_dollars), "$", write_to_log=True)
    ...
    # Not enough liquidity : inform the user, disable the token and break out of this loop
        ...
        printt_warn("- You have set MINIMUM_LIQUIDITY_IN_DOLLARS  =",
                    token['MINIMUM_LIQUIDITY_IN_DOLLARS'], "$", write_to_log=True)
        printt_warn("- Liquidity detected for", token['SYMBOL'], "=", "{:.14g}".format(
            liquidity_amount_in_dollars), "$", write_to_log=True)
        printt_warn(
            "------------------------------------------------", write_to_log=True)
        ...
        if response == "y":
            methods_id = ["0xf305d719", "0xe8e33700",
                          "0xf91b3f72", "0xd71a1bc5"]
...
def wait_for_open_trade(token, inToken, outToken, parameter):
    ...
    if token['WAIT_FOR_OPEN_TRADE'] == 'true' or token['WAIT_FOR_OPEN_TRADE'] == 'true_after_buy_tx_failed':
        ...
        printt("1/ Bot will scan mempool to detect Enable Trading functions",
               write_to_log=True)
        printt("2/ Bot will wait for price to move before making a BUY order",
               write_to_log=True)
        printt(" ", write_to_log=False)
    ...
    if token['WAIT_FOR_OPEN_TRADE'] == 'mempool' or token['WAIT_FOR_OPEN_TRADE'] == 'mempool_after_buy_tx_failed':
        ...
        printt("WAIT_FOR_OPEN_TRADE is enabled", write_to_log=True)
        printt("", write_to_log=True)
        printt("It will scan mempool to detect Enable Trading functions",
               write_to_log=True)
        printt("------------------------------------------------------------------------------------------------------------------------------", write_to_log=True)
    ...
    token['_PREVIOUS_QUOTE'] = check_price(inToken, outToken, token['USECUSTOMBASEPAIR'], token['LIQUIDITYINNATIVETOKEN'], int(
        token['_CONTRACT_DECIMALS']), int(token['_BASE_DECIMALS']))

    tx_filter = client.eth.filter(
        {"filter_params": "pending", "address": inToken})

    # the methodIDs below are openTrading() - like methods. Check below for examples
    list_of_methodId = ["0x8a8c523c", "0x0d295980", "0xbccce037", "0x4efac329", "0x7b9e987a", "0x6533e038", "0x8f70ccf7", "0xa6334231", "0x48dfea0a", "0xc818c280", "0xade87098",
                        "0x0099d386", "0xfb201b1d", "0x293230b8", "0x68c5111a", "0xc49b9a80", "0xc00f04d1", "0xcd2a11be", "0xa0ac5e19", "0x1d97b7cd", "0xf275f64b", "0x5e83ae76", "0x82aa7c68"]
    ...
    while openTrade == False:
        ...
        if token['WAIT_FOR_OPEN_TRADE'] == 'true' or token['WAIT_FOR_OPEN_TRADE'] == 'true_no_message' or token['WAIT_FOR_OPEN_TRADE'] == 'true_after_buy_tx_failed' or token['WAIT_FOR_OPEN_TRADE'] == 'true_after_buy_tx_failed_no_message':
            pprice = check_price(inToken, outToken, token['USECUSTOMBASEPAIR'], token['LIQUIDITYINNATIVETOKEN'], int(
                token['_CONTRACT_DECIMALS']), int(token['_BASE_DECIMALS']))

            if pprice != float(token['_PREVIOUS_QUOTE']):
                token['_TRADING_IS_ON'] = True
                printt_ok("Token price:", pprice,
                          "--> IT HAS MOVED :)", write_to_log=True)
                printt_ok(
                    "PRICE HAS MOVED --> trading is enabled --> Bot will buy", write_to_log=True)
                break
        ...
        try:
            for tx_event in tx_filter.get_new_entries():
                ...
                if txFunction.lower() in list_of_methodId:
                    ...
                    token['_GAS_IS_CALCULATED'] = True
                    token['_GAS_TO_USE'] = int(
                        txHashDetails.gasPrice) / 1000000000
                    printt_ok(
                        "OPEN TRADE FUNCTION DETECTED --> Trading is enabled --> let's buy!", write_to_log=True)
                    printt_ok("MethodID: ", txFunction, " Block: ",
                              tx_event['blockNumber'], " Found Signal", "in txHash:", txHash.hex(), write_to_log=True)
                    printt_ok(
                        "GAS will be the same as detected Tx --> GAS=", token['_GAS_TO_USE'])
                    break
                else:
                    printt("Found something in mempool - MethodID: ",
                           txFunction, " Block: ", tx_event['blockNumber'])
        except Exception as e:
            printt_err(
                "Error detected. It can happen with Public node : private node is recommended. Still, let's continue.")
            continue
...
def build_sell_conditions(token_dict, condition, show_message):
    ...
    if float(token_dict['_TOKEN_BALANCE']) > 0 and (token_dict['_TOKEN_BALANCE'] != token_dict['_PREVIOUS_TOKEN_BALANCE']):
        if token_dict['KIND_OF_SWAP'] == 'base':
            token_dict['_COST_PER_TOKEN'] = float(token_dict['BUYAMOUNTINBASE']) / float(
                (token_dict['_TOKEN_BALANCE'] - token_dict['_PREVIOUS_TOKEN_BALANCE']))
        elif token_dict['KIND_OF_SWAP'] == 'tokens':
            token_dict['_COST_PER_TOKEN'] = float(
                token_dict['_BASE_USED_FOR_TX']) / float(token_dict['BUYAMOUNTINTOKEN'])
        else:
            printt_err("Wrong value in KIND_OF_SWAP parameter")
    printt_debug(token_dict['SYMBOL'], "cost per token was: ",
                 token_dict['_COST_PER_TOKEN'])
        ...
        if condition == 'before_buy':
            if show_message == "show_message":
                ...
                printt_err(
                    "--------------------------------------------------------------------------------------------------", write_to_log=False)
                printt_err(
                    "    DO NOT CLOSE THE BOT after BUY order is made, or your calculated SELLPRICE will be lost!", write_to_log=False)
                printt_err(
                    "--------------------------------------------------------------------------------------------------", write_to_log=False)
            token_dict['_CALCULATED_SELLPRICEINBASE'] = 99999
        else:
            token_dict['_CALCULATED_SELLPRICEINBASE'] = token_dict['_COST_PER_TOKEN'] * \
                (float(sell) / 100)
            printt_info("")
            printt_info(token_dict['SYMBOL'], " cost per token was: ",
                        token_dict['_COST_PER_TOKEN'], write_to_log=True)
            printt_info("--> SELLPRICEINBASE = ", token_dict['SELLPRICEINBASE'], "*",
                        token_dict['_COST_PER_TOKEN'], "= ", token_dict['_CALCULATED_SELLPRICEINBASE'], write_to_log=True)
    ...
    if re.search('^(\d+\.){0,1}\d+%$', str(stop)):
        ...
        if condition == 'before_buy':
            ...
            token_dict['_CALCULATED_STOPLOSSPRICEINBASE'] = token_dict['_COST_PER_TOKEN'] * \
                (float(stop) / 100)
            printt_info("--> STOPLOSSPRICEINBASE = ", token_dict['STOPLOSSPRICEINBASE'], "*",
                        token_dict['_COST_PER_TOKEN'], "= ", token_dict['_CALCULATED_STOPLOSSPRICEINBASE'])
            printt_info("")
    ...
    printt_debug("1111 token_dict['_CALCULATED_SELLPRICEINBASE']    :",
                 token_dict['_CALCULATED_SELLPRICEINBASE'])
    printt_debug("1111 token_dict['_CALCULATED_STOPLOSSPRICEINBASE']:",
                 token_dict['_CALCULATED_STOPLOSSPRICEINBASE'])
...
def check_liquidity_amount(token, DECIMALS_OUT, DECIMALS_weth):
    ...
    if token["LIQUIDITYINNATIVETOKEN"] == 'true':
        ...
        liquidity_amount = check_pool(
            inToken, weth, token['_LIQUIDITY_DECIMALS'])
        liquidity_amount_in_dollars = float(
            liquidity_amount) * float(token['_BASE_PRICE'])
        printt("Current", token['_PAIR_SYMBOL'], "Liquidity =",
               "{:.2f}".format(liquidity_amount_in_dollars), "$")
        printt("")

        if float(token['MINIMUM_LIQUIDITY_IN_DOLLARS']) <= float(liquidity_amount_in_dollars):
            printt_ok("MINIMUM_LIQUIDITY_IN_DOLLARS parameter =", int(
                token['MINIMUM_LIQUIDITY_IN_DOLLARS']), " --> Enough liquidity detected : Buy Signal Found!")
            return 1
        ...
        # This position isn't looking good. Inform the user, disable the token and break out of this loop
            ...
            printt_warn("- You have set MINIMUM_LIQUIDITY_IN_DOLLARS  =",
                        token['MINIMUM_LIQUIDITY_IN_DOLLARS'], "$", write_to_log=True)
            printt_warn("- Liquidity detected for", token['SYMBOL'], "=", "{:.2f}".format(
                liquidity_amount_in_dollars), "$", write_to_log=True)
            printt_warn("--> Bot will not buy and disable token",
                        write_to_log=True)
            printt_warn(
                "------------------------------------------------", write_to_log=True)
            token['ENABLED'] = 'false'
    ...
    if token["LIQUIDITYINNATIVETOKEN"] == 'false' and token["USECUSTOMBASEPAIR"] == 'true':
        ...
        liquidity_amount = check_pool(
            inToken, outToken, token['_LIQUIDITY_DECIMALS'])
        ...
        custom_base_price_in_base = calculate_custom_base_price(
            outToken, DECIMALS_OUT, DECIMALS_weth)
        ...
        printt("Current", token['SYMBOL'], "Liquidity =",
               "{:.6f}".format(liquidity_amount_in_dollars), "$")
...
def check_liquidity_amount_mempool(token, amount_detected):
    ...
    if token["LIQUIDITYINNATIVETOKEN"] == 'true':
        ...
        liquidity_amount_in_dollars = float(
            amount_detected) * float(token['_BASE_PRICE'])
        printt("We detected ", token['_PAIR_SYMBOL'], "Liquidity =", "{:.2f}".format(
            liquidity_amount_in_dollars), "$", write_to_log=True)
        printt("")
        ...
        if float(token['MINIMUM_LIQUIDITY_IN_DOLLARS']) <= float(liquidity_amount_in_dollars):
            ...
            printt_ok("- You have set MINIMUM_LIQUIDITY_IN_DOLLARS  =",
                      token['MINIMUM_LIQUIDITY_IN_DOLLARS'], "$", write_to_log=True)
            printt_ok("- Liquidity added for", token['SYMBOL'], "=", "{:.2f}".format(
                liquidity_amount_in_dollars), "$", write_to_log=True)
            printt_ok("--> Let's buy!", write_to_log=True)
            printt_ok(
                "------------------------------------------------", write_to_log=True)
    ...
    if token["LIQUIDITYINNATIVETOKEN"] == 'false' and token["USECUSTOMBASEPAIR"] == 'true':
        ...
        custom_base_price_in_base = token['_BASE_PRICE']

        # 2/ convert this Custom Base token price in $
        custom_base_price_in_dollars = float(
            custom_base_price_in_base) * float(token['_BASE_PRICE'])
        liquidity_amount_in_dollars = float(
            amount_detected) * float(custom_base_price_in_dollars)
...
def check_price(inToken, outToken, custom, routing, DECIMALS_IN, DECIMALS_OUT):
    ...
    if routing == 'true':
        ...
        if outToken != weth:
            ...
            price_check = routerContract.functions.getAmountsOut(
                1 * DECIMALS_IN, [inToken, weth, outToken]).call()[-1]
            ...
            price_check = routerContract.functions.getAmountsOut(
                1 * DECIMALS_IN, [inToken, weth]).call()[-1]
        ...
        if outToken != weth:
            ...
            # USECUSTOMBASEPAIR = true and token put in BASEADDRESS is different from WBNB / WETH
            price_check = routerContract.functions.getAmountsOut(
                1 * DECIMALS_IN, [inToken, outToken]).call()[-1]
...
def check_precise_price(inToken, outToken, DECIMALS_weth, DECIMALS_IN, DECIMALS_OUT):
    ...
    if outToken != weth:
        ...
        if not ORDER_HASH[pair_address]:
            tokenPrice1 = Decimal(
                (reserves[0] / DECIMALS_weth) / (reserves[1] / DECIMALS_IN))
        else:
            tokenPrice1 = Decimal(
                (reserves[1] / DECIMALS_weth) / (reserves[0] / DECIMALS_IN))
        ...
        if not ORDER_HASH[pair_address]:
            tokenPrice2 = Decimal(
                (reserves[0] / DECIMALS_weth) / (reserves[1] / DECIMALS_OUT))
        else:
            tokenPrice2 = Decimal(
                (reserves[1] / DECIMALS_weth) / (reserves[0] / DECIMALS_OUT))
        ...
        if not ORDER_HASH[pair_address]:
            tokenPrice = Decimal(
                (reserves[0] / DECIMALS_OUT) / (reserves[1] / DECIMALS_IN))
        else:
            tokenPrice = Decimal(
                (reserves[1] / DECIMALS_OUT) / (reserves[0] / DECIMALS_IN))
...
def check_precise_price_new(inToken, outToken, DECIMALS_weth, DECIMALS_IN, DECIMALS_OUT):
    ...
    if pair_address != '0x0000000000000000000000000000000000000000':
        printt_debug(
            "ENTER check_precise_price_new condition 0 direct pool LP")
        # First step : calculates the price of token in ETH/BNB
...
def calculate_base_price():
    ...
    if base_symbol == "BNB" or base_symbol == "BNB ":
        ...
        reserves = pair_contract.functions.getReserves().call()
        basePrice = Decimal(
            (reserves[1] / DECIMALS_STABLES) / (reserves[0] / DECIMALS_BNB))
        ...
        reserves = pair_contract.functions.getReserves().call()
        basePrice = Decimal(
            (reserves[1] / DECIMALS_STABLES) / (reserves[0] / DECIMALS_ETH))
        ...
        reserves = pair_contract.functions.getReserves().call()
        basePrice = Decimal(
            (reserves[0] / DECIMALS_STABLES) / (reserves[1] / DECIMALS_ETH))
    ...
    for stable_token in settings['_STABLE_BASES']:
        settings['_STABLE_BASES'][stable_token]['multiplier'] = float(
            basePrice)
...
def calculate_base_balance(token):
    ...
    try:
        ...
        printt_err("ERROR with your node : please check logs.",
                   write_to_log=True)
        logger1.exception(e)
        sys.exit()
    ...
    if eth_balance < minimumbalance:
        ...
        printt_err(
            "We know it can seem a lot, but the smart contracts used by Exchanges have automatic controls of minimal balance.")
        sleep(10)
        exit(1)
    ...
    if token['USECUSTOMBASEPAIR'].lower() == 'false':
        ...
        printt_debug(
            "token['_BASE_BALANCE'] in calculate_base_balance:", token['_BASE_BALANCE'])
    else:
        address = Web3.toChecksumAddress(token['BASEADDRESS'])
        DECIMALS = decimals(address)
        balance_check = check_balance(
            token['BASEADDRESS'], token['BASESYMBOL'])
...
def calculate_gas(token):
    ...
    if token['GAS'] == 'boost' or token['GAS'] == 'BOOST' or token['GAS'] == 'Boost':
        ...
        token['_GAS_TO_USE'] = (
            gas_price * ((int(token['BOOSTPERCENT'])) / 100)) + gas_price
        printt_info(
            "BUY/SELL transaction will be created with GAS = Fast price * BOOSTPERCENT")
        printt_info("")
        ...
        token['_GAS_TO_USE'] = (
            gas_price * ((int(token['BOOSTPERCENT'])) / 100)) + gas_price
        printt_info(
            "- BUY  transaction will be created with same GAS as liquidity adding Tx detected")
        printt_info(
            "- SELL transaction will be created with gas you entered in tokens.json")
        printt_info("")
    else:
        printt_info("")
        printt_info(
            "BUY/SELL transaction will be created with GAS =", token['GAS'])
        ...
        if verified['ABI'] != 'Contract source code not verified':
            ...
            for contract in _contract:
                ...
                if 'TransferHelper' in verified['ABI'] or 'TransferHelper' in source:
                    print(
                        "Our Rug code Checker has found that this contract contains has Honeypot Function Found in Code")
                    mint = True
                elif 'IPayable' in verified['ABI'] or 'IPayable' in source:
                    print(
                        "Our Rug code Checker has found that this contract was made by token generator")
                    mint = True
                elif 'HelloBEP20' in verified['ABI'] or 'HelloBEP20' in source:
                    print(
                        "Our Rug code Checker has found that this contract contains has vittominacori.github.io Generator")
                    mint = True
                elif 'BEP20TOKEN' in verified['ABI'] or 'BEP20TOKEN' in source:
                    print(
                        "Our Rug code Checker has found that this contract contains has Common Scam Contract using BEP20TOKEN Code")
                    mint = True
                elif 'clearCNDAO' in verified['ABI'] or 'clearCNDAO' in source:
                    print(
                        "Our Rug code Checker has found that this contract contains has clearCNDAO function Found in Contract")
                    mint = True
                    ...
                    print(
                        "Our Rug code Checker has found that this contract contains has Remove Liquidty Function")
                    mint = True
                elif 'transferFromMiddleware' in verified['ABI'] or 'transferFromMiddleware' in source:
                    print(
                        "Our Rug code Checker has found that this contract contains has Transfer Middleware Fucntion")
                    mint = True
                elif 'ratchetClank' in verified['ABI'] or 'ratchetClank' in source:
                    print(
                        "Our Rug code Checker has found that this contract contains has Ratchet & Clank Scam Function")
                    mint = True
                elif 'require(from == _owner' in source:
                    print(
                        "Our Rug code Checker has found that this contract contains has Tranfer Function Scam ")
                    mint = True
                    ...
                    print(
                        "Our Rug code Checker has found that this contract contains has Issue Redeem Mint Function Scam ...")
                    mint = True
...
def make_the_buy(inToken, outToken, buynumber, pwd, amount, gas, gaslimit, gaspriority, routing, custom, slippage, DECIMALS, nonce):
    ...
    if custom.lower() == 'false':
        ...
        if routing.lower() == 'false':
            ...
            if modified == True:
                ...
                if settings["EXCHANGE"].lower() == 'koffeeswap':
                    ...
                    printt_debug("make_the_buy condition 11",
                                 write_to_log=True)
        ...
        if inToken == weth:
            ...
            amount_out = routerContract.functions.getAmountsOut(
                amount, [weth, outToken]).call()[-1]
            ...
            if routing.lower() == 'true':
                ...
                if modified == True:
                    if settings["EXCHANGE"].lower() == 'koffeeswap':
                        printt_debug("make_the_buy condition 13",
                                     write_to_log=True)
                        ...
                        printt_debug("make_the_buy condition 14",
                                     write_to_log=True)
                        ...
                        printt_debug("make_the_buy condition 15",
                                     write_to_log=True)
                    ...
                    # Base Pair different from weth
                    # Exchange different from Uniswap
                    printt_debug("make_the_buy condition 10",
                                 write_to_log=True)
    ...
    if buynumber == 0:
        ...
        signed_txn = client.eth.account.signTransaction(
            transaction, private_key=settings['PRIVATEKEY'])
    if buynumber == 1:
        settings['PRIVATEKEY2'] = settings['PRIVATEKEY2'].replace(
            'aes:', "", 1)
        settings['PRIVATEKEY2'] = cryptocode.decrypt(
            settings['PRIVATEKEY2'], pwd)
        signed_txn = client.eth.account.signTransaction(
            transaction, private_key=settings['PRIVATEKEY2'])
    if buynumber == 2:
        settings['PRIVATEKEY3'] = settings['PRIVATEKEY3'].replace(
            'aes:', "", 1)
        settings['PRIVATEKEY3'] = cryptocode.decrypt(
            settings['PRIVATEKEY3'], pwd)
        signed_txn = client.eth.account.signTransaction(
            transaction, private_key=settings['PRIVATEKEY3'])
    if buynumber == 3:
        settings['PRIVATEKEY4'] = settings['PRIVATEKEY4'].replace(
            'aes:', "", 1)
        settings['PRIVATEKEY4'] = cryptocode.decrypt(
            settings['PRIVATEKEY4'], pwd)
        signed_txn = client.eth.account.signTransaction(
            transaction, private_key=settings['PRIVATEKEY4'])
    if buynumber == 4:
        settings['PRIVATEKEY5'] = settings['PRIVATEKEY5'].replace(
            'aes:', "", 1)
        settings['PRIVATEKEY5'] = cryptocode.decrypt(
            settings['PRIVATEKEY5'], pwd)
        signed_txn = client.eth.account.signTransaction(
            transaction, private_key=settings['PRIVATEKEY5'])
...
def make_the_buy_exact_tokens(token_dict, inToken, outToken, buynumber, pwd, gaslimit, routing, custom, slippage, DECIMALS_FOR_AMOUNT, basebalance, nonce):
    ...
    printt_debug("make_the_buy_exact_tokens amount:",
                 amount, write_to_log=True)
    ...
    if custom.lower() == 'false':
        ...
        if routing.lower() == 'false':
            ...
            # LIQUIDITYINNATIVETOKEN = true
            # USECUSTOMBASEPAIR = false
            amount_in = routerContract.functions.getAmountsIn(
                amount, [weth, outToken]).call()[0]
            ...
            if amount_in > base_balance_before_buy:
                printt_err("- You have          ", base_balance_before_buy /
                           DECIMALS, base_symbol, "in your wallet ")
                printt_err("- But you would need", amount_in / DECIMALS,
                           base_symbol, "to buy this amount of tokens ")
            ...
            if (amount_in / DECIMALS) > token_dict['MAX_BASE_AMOUNT_PER_EXACT_TOKENS_TRANSACTION']:
                printt_err("- This transaction would need",
                           "{:.6f}".format(amount_in / DECIMALS), base_symbol, "to be done")
                printt_err("- And you asked the bot not to use more than",
                           token_dict['MAX_BASE_AMOUNT_PER_EXACT_TOKENS_TRANSACTION'], base_symbol)
                printt_err("--> buy cancelled ")
                sleep(10)
                sys.exit()
            ...
            printt_ok("")
            printt_warn(
                "WARNING : buying exact amount of tokens only works with LIQUIDITYINNATIVETOKEN = true and USECUSTOMBASEPAIR = false")
            printt_ok("")
            printt_ok("Amount of tokens to buy        :", amount /
                      DECIMALS_FOR_AMOUNT, token_symbol, write_to_log=True)
            printt_ok("Amount of base token to be used:", amount_in /
                      DECIMALS, base_symbol, write_to_log=True)
            printt_ok("Current Base token balance     :",
                      base_balance_before_buy / DECIMALS, base_symbol, write_to_log=True)
            printt_ok(
                "(be careful you must have enough to pay fees in addition to this)", write_to_log=True)
            printt_ok(
                "-----------------------------------------------------------", write_to_log=True)
            ...
            if modified == True:
                ...
                if settings["EXCHANGE"].lower() == 'koffeeswap':
                    printt_debug(
                        "make_the_buy_exact_tokens condition 1", write_to_log=True)
                    ...
                    printt_debug(
                        "make_the_buy_exact_tokens condition 2", write_to_log=True)
                    ...
                    printt_debug(
                        "make_the_buy_exact_tokens condition 11", write_to_log=True)
                ...
                if settings["EXCHANGE"].lower() == 'uniswap' or settings["EXCHANGE"].lower() == 'uniswaptestnet':
                    printt_debug(
                        "make_the_buy_exact_tokens condition 3", write_to_log=True)
                    ...
                    printt_debug(
                        "make_the_buy_exact_tokens condition 4", write_to_log=True)
        ...
        printt("")
        printt_err(
            "Sorry, KIND_OF_SWAP = tokens is only available when liquidity is added in native token (BNB/ETH...).")
        printt_err(
            "If you want the Sniper to buy when liquidity is not added in native token, please use KIND_OF_SWAP = base.")
        printt_err("Exiting")
        sleep(10)
        sys.exit()
...
def wait_for_tx(token_dict, tx_hash, address, max_wait_time=60):
    ...
    while got_receipt == False and exit_timestamp > time():
        ...
        if loop_iterations == 0:
            ...
            print(style.INFO, " (waiting ", max_wait_time, " seconds)",
                  style.RESET, sep='', end="", flush=True)
        else:
...
def preapprove_base(token):
    ...
    printt("LimitSwap will now check approval of your Base or Custom Base for",
           token['SYMBOL'], ", to ensure you can use it")

    if token['USECUSTOMBASEPAIR'].lower() == 'false':
        balanceweth = Web3.fromWei(client.eth.getBalance(
            settings['WALLETADDRESS']), 'ether')
        check_approval(token, weth, balanceweth *
                       decimals(weth), 'base_approve')
    else:
        balancebase = Web3.fromWei(check_balance(
            token['BASEADDRESS'], token['BASESYMBOL'], display_quantity=False), 'ether')
        check_approval(token, token['BASEADDRESS'], balancebase *
                       decimals(token['BASEADDRESS']), 'base_approve')
...
def buy(token_dict, inToken, outToken, pwd):
    ...
    gaslimit = int(token_dict['GASLIMIT'])
    custom = token_dict['USECUSTOMBASEPAIR']
    routing = token_dict['LIQUIDITYINNATIVETOKEN']
    gaspriority = token_dict['GASPRIORITY_FOR_ETH_ONLY']
    ...
    printt_debug("debug _FAILED_TRANSACTIONS:",
                 token_dict['_FAILED_TRANSACTIONS'])
    if token_dict['_FAILED_TRANSACTIONS'] >= int(token_dict['MAX_FAILED_TRANSACTIONS_IN_A_ROW']):
        ...
        printt_err(
            "This token has reached maximum FAILED SIMULTANIOUS TRANSACTIONS", write_to_log=True)
        printt_err(
            "---------------------------------------------------------------", write_to_log=True)
        token_dict['ENABLED'] = 'false'
        return False

    # Check for amount of success transactions before buy (MAX_SUCCESS_TRANSACTIONS_IN_A_ROW parameter)
    printt_debug("debug _SUCCESS_TRANSACTIONS:",
                 token_dict['_SUCCESS_TRANSACTIONS'])
    if token_dict['_SUCCESS_TRANSACTIONS'] >= int(token_dict['MAX_SUCCESS_TRANSACTIONS_IN_A_ROW']):
        ...
        printt_ok(
            "This token has reached maximum SUCCESS SIMULTANIOUS TRANSACTIONS", write_to_log=True)
        printt_ok(
            "---------------------------------------------------------------", write_to_log=True)
        token_dict['ENABLED'] = 'false'
        return False
    ...
    if balance > Decimal(amount):
        ...
        if token_dict['_GAS_TO_USE'] > token_dict['MAX_GAS']:
            printt_err("GAS = ", token_dict['_GAS_TO_USE'], "is superior to your MAX_GAS parameter (=",
                       token_dict['MAX_GAS'], ") --> bot do not buy", )
            token_dict['ENABLED'] = 'false'
            return False
        ...
        if multiplebuys.lower() == 'true':
            ...
            while True:
                if buynumber < amount_of_buys:
                    ...
                    if token_dict['KIND_OF_SWAP'] == 'tokens':
                        make_the_buy_exact_tokens(token_dict, inToken, outToken, buynumber, pwd,
                                                  gaslimit, routing, custom, slippage, CONTRACT_DECIMALS, balance, nonce)
                    else:
                        make_the_buy(inToken, outToken, buynumber, pwd, amount, int(
                            token_dict['_GAS_TO_USE']), gaslimit, gaspriority, routing, custom, slippage, CONTRACT_DECIMALS, nonce)
                    buynumber += 1
                else:
            ...
            if token_dict['KIND_OF_SWAP'] == 'tokens':
                tx_hash = make_the_buy_exact_tokens(
                    token_dict, inToken, outToken, buynumber, pwd, gaslimit, routing, custom, slippage, CONTRACT_DECIMALS, balance, nonce)
            else:
                tx_hash = make_the_buy(inToken, outToken, buynumber, pwd, amount, int(
                    token_dict['_GAS_TO_USE']), gaslimit, gaspriority, routing, custom, slippage, CONTRACT_DECIMALS, nonce)
        ...
        printt_debug(token_dict)
        printt_err("You don't have enough", token_dict['BASESYMBOL'],
                   "in your wallet to make the BUY order of", token_dict['SYMBOL'], "--> bot do not buy", )
...
def sell(token_dict, inToken, outToken):
    ...
    printt_debug("debug 2419 _FAILED_TRANSACTIONS:",
                 token_dict['_FAILED_TRANSACTIONS'])
    if token_dict['_FAILED_TRANSACTIONS'] >= int(token_dict['MAX_FAILED_TRANSACTIONS_IN_A_ROW']):
        printt_err(
            "---------------------------------------------------------------")
        ...
        printt_err(
            "This token has reached maximum FAILED SIMULTANIOUS TRANSACTIONS")
        printt_err(
            "---------------------------------------------------------------")
        token_dict['ENABLED'] = 'false'
        return False
    ...
    if balance >= 1:
        ...
        if token_dict['_GAS_TO_USE'] > token_dict['MAX_GAS']:
            printt_err("GAS = ", token_dict['_GAS_TO_USE'], "is superior to your MAX_GAS parameter (=",
                       token_dict['MAX_GAS'], ") --> bot do not sell and disable token", )
            token_dict['ENABLED'] = 'false'
            return False
        ...
        if amount == 'all' or amount == 'ALL' or amount == 'All':
            ...
            if amount <= 0:
                # Example 4
                printt_err(
                    "Not enough left to sell, would bust moonbag. Disabling the trade of this token.")
                amount = 0
                token_dict['ENABLED'] = 'false'
                return False
        else:
            ...
            if balance < amount:
                ...
                if (balance - amount) >= moonbag:
                    ...
                    printt_err(
                        "Balance = Moonbag. Disabling the trade of this token.")
                    amount = 0
                    token_dict['ENABLED'] = 'false'
                    return False
                else:
        ...
        if custom.lower() == 'false':
            ...
            if outToken == weth:
                ...
                if routing.lower() == 'false' and outToken != weth:
                    ...
                    # USECUSTOMBASEPAIR = true
                    amount_out = routerContract.functions.getAmountsOut(
                        amount, [inToken, outToken]).call()[-1]
                    ...
                    amount_out = routerContract.functions.getAmountsOut(
                        amount, [inToken, weth, outToken]).call()[-1]
        ...
        printt("Your", token_dict['SYMBOL'],
               "balance is very small (< 1), so LimitSwap will not try to sell, so as not to waste fees.")
        return False
...
def benchmark():
    ...
    rounds = 60
    printt("Benchmark running, we will do", rounds,
           "tests. Please wait a few seconds...")

# Check RPC Node latency
    k = 0
    try:
        if my_provider[0].lower() == 'h' or my_provider[0].lower() == 'w':
            provider = my_provider.replace(
                'wss://', 'https://').replace('ws://', 'http://')
            for i in range(5):
                response = requests.post(provider)
                k = k + response.elapsed.total_seconds()
                sleep(0.05)
            printt('RPC Node average latency :', str(
                int((k/5)*1000)), 'ms', write_to_log=True)
    except Exception:
        pass
...
# Check 'check_price' function speed
    ...
    if token[0]['USECUSTOMBASEPAIR'] == 'true':
        ...
        token[0]['_LIQUIDITY_DECIMALS'] = int(
            decimals(token[0]['BASEADDRESS']))
    ...
    for i in range(rounds):
        try:
            tmp = check_price(inToken, outToken, token[0]['USECUSTOMBASEPAIR'], token[0]
                              ['LIQUIDITYINNATIVETOKEN'], token[0]['_CONTRACT_DECIMALS'], token[0]['_BASE_DECIMALS'])
        except Exception:
            pass

    end_time = time()
    printt('Check_price function     :', round((rounds/(end_time - start_time)), 2),
           'query/s Total:', round((end_time - start_time), 2), "s", write_to_log=True)
...
# Check 'check_precise_price' function speed
    ...
    for i in range(rounds):
        try:
            tmp = check_precise_price(
                inToken, outToken, token[0]['_WETH_DECIMALS'], token[0]['_CONTRACT_DECIMALS'], token[0]['_BASE_DECIMALS'])
        except Exception:
            pass
    end_time = time()
    printt('Check_precise_price func :', round((rounds/(end_time - start_time)), 2),
           'query/s Total:', round((end_time - start_time), 2), "s", write_to_log=True)
...
# Check 'check_pool' function speed
    ...
    for i in range(rounds):
        try:
            ...
            pass
    end_time = time()
    printt('Check_pool function      :', round((rounds/(end_time - start_time)), 2),
           'query/s Total:', round((end_time - start_time), 2), "s", write_to_log=True)
    ...
    try:
        ...
        for token in tokens:
            ...
            if token['MULTIPLEBUYS'].lower() == 'true' and token['KIND_OF_SWAP'].lower() == 'tokens':
                printt_err(
                    "MULTIPLEBUYS is only compatible with KIND_OF_SWAP = base... Sorry.")
                sys.exit()
            ...
            if token['KIND_OF_SWAP'].lower() == 'tokens' and token['MAX_BASE_AMOUNT_PER_EXACT_TOKENS_TRANSACTION'] == 0:
                printt_err(
                    "You have selected KIND_OF_SWAP = tokens, so you must enter a value in MAX_BASE_AMOUNT_PER_EXACT_TOKENS_TRANSACTION")
                sys.exit()

            if token['BUYAMOUNTINBASE'] == 0:
                printt_err("You need to enter a BUY amount")
                sys.exit()
            ...
            printt_debug("Pre-calculations for token:",
                         token['ADDRESS'], ": Gas / Decimals / Balance / RugDoc check")

            token['_CONTRACT_DECIMALS'] = int(decimals(token['ADDRESS']))
            printt_debug("token['_CONTRACT_DECIMALS']:",
                         token['_CONTRACT_DECIMALS'])
            ...
            printt_debug("token['_BASE_DECIMALS']    :",
                         token['_BASE_DECIMALS'])

            # Calculates _LIQUIDITY_DECIMALS
            if token['USECUSTOMBASEPAIR'] == 'true' and token['LIQUIDITYINNATIVETOKEN'] == 'false':
                token['_LIQUIDITY_DECIMALS'] = int(
                    decimals(token['BASEADDRESS']))
            else:
                token['_LIQUIDITY_DECIMALS'] = int(decimals(weth))
            printt_debug("token['_LIQUIDITY_DECIMALS']    :",
                         token['_LIQUIDITY_DECIMALS'])

            # Calculates _WETH_DECIMALS
            token['_WETH_DECIMALS'] = int(decimals(weth))
            printt_debug("token['_WETH_DECIMALS']    :",
                         token['_WETH_DECIMALS'])
            ...
            printt_debug("token['_CALCULATED_SELLPRICEINBASE']:",
                         token['_CALCULATED_SELLPRICEINBASE'])
            printt_debug("token['_CALCULATED_STOPLOSSPRICEINBASE']:",
                         token['_CALCULATED_STOPLOSSPRICEINBASE'])

            # Determine token name + base symbol to display
            if token['USECUSTOMBASEPAIR'] == 'true':
                token['_PAIR_TO_DISPLAY'] = token['SYMBOL'] + \
                    "/" + token['BASESYMBOL']
            else:
                token['_PAIR_TO_DISPLAY'] = token['SYMBOL'] + "/" + base_symbol

            # let's define _OUT_TOKEN (the token you want to TRADE WITH (example : ETH or USDT))
            # it depends of which liquidity was detected
            if token['USECUSTOMBASEPAIR'] == 'true':
                token['_OUT_TOKEN'] = Web3.toChecksumAddress(
                    token['BASEADDRESS'])
            else:
                token['_OUT_TOKEN'] = weth

            # Check to see if we have any tokens in our wallet already
            token['_TOKEN_BALANCE'] = check_balance(
                token['ADDRESS'], token['SYMBOL'], display_quantity=False) / token['_CONTRACT_DECIMALS']
            token['_PREVIOUS_TOKEN_BALANCE'] = token['_TOKEN_BALANCE']
            if token['_TOKEN_BALANCE'] > 0:
                printt("")
                printt("Your wallet already owns : ",
                       token['_TOKEN_BALANCE'], token['SYMBOL'], write_to_log=True)
                if token['_TOKEN_BALANCE'] >= float(token['MAXTOKENS']):
                    token['_REACHED_MAX_TOKENS'] = True
                    printt_warn("You have reached MAXTOKENS for token ",
                                token['SYMBOL'], "--> bot stops to buy", write_to_log=True)
                    printt("")
                else:
        ...
        while True:
            ...
            if load_token_file_increment > 1:
                ...
                tokens_file_modified_time = os.path.getmtime(
                    command_line_args.tokens)
                    ...
                    if command_line_args.password_on_change or settings['PASSWORD_ON_CHANGE'] == 'true':
                        ...
                        if pkpassword != userpassword:
                            printt_err(
                                "ERROR: Your private key decryption password is incorrect")
                            printt_err(
                                "Please re-launch the bot and try again")
                            sleep(10)
                            sys.exit()
            ...
            for token in tokens:
                ...
                if token['ENABLED'] == 'true':
                    ...
                    # MEMPOOL SCAN
                    #
                    # if we haven't reached MAXTOKENS
                    # and no BUY has been made yet
                    # --> let's scan mempool !
                    #
                    if token['_TOKEN_BALANCE'] < float(token['MAXTOKENS']) and token['_BUY_IS_MADE'] == False:
                        #
                        ...
                        if settings['MEMPOOL_METHOD'] == 'public_node':
                            AddLiquidity_TxHash = scan_mempool_public_node(
                                token)
                            buy_TxHash = buy(
                                token, token['_OUT_TOKEN'], token['_IN_TOKEN'], userpassword)
                        elif settings['MEMPOOL_METHOD'] == 'private_node':
                            AddLiquidity_TxHash, buy_TxHash = scan_mempool_private_node(
                                token, methods_id)
                        elif settings['MEMPOOL_METHOD'] == 'wip':
                            AddLiquidity_TxHash = scan_mempool_private_node_work_in_progress(
                                token, methods_id)
                        elif settings['MEMPOOL_METHOD'] == 'pinksale':
                            AddLiquidity_TxHash, buy_TxHash = scan_mempool_pinksale_private_node(
                                token)
                        else:
                            printt_err(
                                "Wrong value in MEMPOOL_METHOD setting.")
                            sleep(10)
                            sys.exit()

                        if buy_TxHash != False:
                            txbuyresult = wait_for_tx(
                                token, buy_TxHash, token['ADDRESS'])
                            printt_debug(
                                "wait_for_tx result is : ", txbuyresult)
                            if txbuyresult != 1:
                                ...
                                printt_err("-------------------------------")
                                printt_err(
                                    "Type of failures and possible causes:")
                                printt_err(
                                ...
                                printt_err(
                                    "- 'Pancake: K'                 --> There are additional fees --> use HASFEES = true")
                                printt_err(
                                ...
                                try:
                                    ...
                                    printt_err(
                                        "An error occured when using Apprise notification --> check your settings.")
                                    logging.exception(e)
                                    logger1.exception(e)
                                ...
                                printt_debug(
                                    "3813 _FAILED_TRANSACTIONS:", token['_FAILED_TRANSACTIONS'])
                                ...
                                if token['WAIT_FOR_OPEN_TRADE'].lower() == 'true_after_buy_tx_failed' or token['WAIT_FOR_OPEN_TRADE'].lower() == 'true_after_buy_tx_failed_no_message' or token['WAIT_FOR_OPEN_TRADE'] == 'mempool_after_buy_tx_failed':
                                    wait_for_open_trade(
                                        token, token['_IN_TOKEN'], token['_OUT_TOKEN'], 'normal')
                                ...
                                printt_ok(
                                    "----------------------------------", write_to_log=True)
                                printt_ok(
                                    "SUCCESS : your buy Tx is confirmed", write_to_log=True)
                                printt_ok("")
                                buy_tx_detail = client.eth.get_transaction(
                                    buy_TxHash)
                                AddLiquidity_tx_detail = client.eth.get_transaction(
                                    AddLiquidity_TxHash)

                                printt_ok(" - Your BUY Block number is     : ",
                                          buy_tx_detail['blockNumber'], write_to_log=True)
                                printt_ok(" - Addliquidity Block number was: ",
                                          AddLiquidity_tx_detail['blockNumber'], write_to_log=True)
                                printt_ok("")

                                if buy_tx_detail['blockNumber'] - AddLiquidity_tx_detail['blockNumber'] == 0:
                                    printt_ok(
                                        "CONGRATS!! You bought in the same block :)", write_to_log=True)
                                else:
                                    printt(
                                        "You bought", buy_tx_detail['blockNumber'] - AddLiquidity_tx_detail['blockNumber'], "blocks later", write_to_log=True)
                                ...
                                printt(
                                    "Let's wait 3s before we check your new balance, to be sure we get correct amount")
                                sleep(3)
                                printt("")
                                ...
                                token['_TOKEN_BALANCE'] = check_balance(
                                    token['_IN_TOKEN'], token['SYMBOL'], display_quantity=True) / DECIMALS
                                printt_ok("", write_to_log=True)
                                printt_ok(
                                    "You bought", token['_TOKEN_BALANCE'] - token['_PREVIOUS_TOKEN_BALANCE'], token['SYMBOL'], "tokens", write_to_log=True)
                                printt_ok(
                                    "----------------------------------", write_to_log=True)
                                ...
                                if (settings['PREAPPROVE'] == 'instantafterbuy' or settings['PREAPPROVE'] == 'true'):
                                    check_approval(
                                        token, token['ADDRESS'], token['_TOKEN_BALANCE'] * DECIMALS, 'preapprove')
                                ...
                                if token['XXX_SECONDS_COOLDOWN_AFTER_BUY_SUCCESS_TX'] != 0:
                                    printt_info("Bot will wait", token['XXX_SECONDS_COOLDOWN_AFTER_BUY_SUCCESS_TX'],
                                                "seconds after BUY, due to XXX_SECONDS_COOLDOWN_AFTER_BUY_SUCCESS_TX parameter", write_to_log=True)
                                    sleep(
                                        token['XXX_SECONDS_COOLDOWN_AFTER_BUY_SUCCESS_TX'])

                                # increment _SUCCESS_TRANSACTIONS amount
                                token['_SUCCESS_TRANSACTIONS'] += 1
                                printt_debug(
                                    "3840 _SUCCESS_TRANSACTIONS:", token['_SUCCESS_TRANSACTIONS'])
                                ...
                                if token['_SUCCESS_TRANSACTIONS'] >= token['MAX_SUCCESS_TRANSACTIONS_IN_A_ROW']:
                                    ...
                                    printt_warn("You have reached MAX_SUCCESS_TRANSACTIONS_IN_A_ROW for",
                                                token['SYMBOL'], "token --> disabling trade", write_to_log=True)

                                printt_info("")
                                printt_info(
                                    "--------------------------------------------")
                                printt_info(
                                    "   Buy is now made --> Entering SELL MODE")
                                printt_info(
                                    "--------------------------------------------")
                                printt_info("")
                                ...
                                build_sell_conditions(
                                    token, 'after_buy', 'show_message')
                    ...
                    #
                    #  SELL MODE
                    #  Buy has been made --> let's SELL this token :)
                    #
                    token['_PREVIOUS_QUOTE'] = token['_QUOTE']

                    if token['LIQUIDITYINNATIVETOKEN'] == 'true':
                        token['_QUOTE'] = check_precise_price(
                            token['_IN_TOKEN'], token['_OUT_TOKEN'], token['_WETH_DECIMALS'], token['_CONTRACT_DECIMALS'], token['_BASE_DECIMALS'])
                    else:
                        ...
                        token['_QUOTE'] = check_price(token['_IN_TOKEN'], token['_OUT_TOKEN'], token['USECUSTOMBASEPAIR'],
                                                      token['LIQUIDITYINNATIVETOKEN'], token['_CONTRACT_DECIMALS'], token['_BASE_DECIMALS'])
                    ...
                    # and token['_REACHED_MAX_TOKENS'] == False:
                    if token['_PREVIOUS_QUOTE'] != 0 and token['_QUOTE'] != 0:
                    ...
                    printt_debug(
                        "_TOKEN_BALANCE 3411", token['_TOKEN_BALANCE'], "for the token:", token['SYMBOL'])
                    ...
                    if isinstance(command_line_args.pump, int) and command_line_args.pump > 0:
                        ...
                        if token['_COST_PER_TOKEN'] == 0 and token['_INFORMED_SELL'] == False:
                            printt_warn(
                                "WARNING: You are running a pump on an already purchased position.")
                            sleep(5)
                        ...
                        if token['_FIRST_SELL_QUOTE'] == 0:
                            ...
                            maximum_gains = token['_ALL_TIME_HIGH'] - \
                                token['_FIRST_SELL_QUOTE']
                            minimum_price = token['_ALL_TIME_HIGH'] - \
                                (command_line_args.pump * 0.01 * maximum_gains)
                            if quote < minimum_price:
                                printt_err(
                                    token['SYMBOL'], "has dropped", command_line_args.pump, "% from it's ATH - SELLING POSITION")
                    ...
                    if price_conditions_met == True:
                        ...
                        printt_ok(
                            "--------------------------------------------------------------")
                        printt_ok(
                            "Sell Signal Found =-= Sell Signal Found =-= Sell Signal Found ")
                        printt_ok("", write_to_log=True)
                        printt_ok(
                            "Sell price in", token['_PAIR_TO_DISPLAY'], ":", log_price, write_to_log=True)
                        printt_ok(
                            "--------------------------------------------------------------")
                        ...
                        if token["MINIMUM_LIQUIDITY_IN_DOLLARS"] != 0:
                            liquidity_result = check_liquidity_amount(
                                token, token['_BASE_DECIMALS'], token['_WETH_DECIMALS'])
                            if liquidity_result == 0:
                                continue
                            else:
                                pass
                        ...
                        tx = sell(
                            token, token['_IN_TOKEN'], token['_OUT_TOKEN'])

                        if tx != False:
                            txsellresult = wait_for_tx(
                            ...
                            if txsellresult != 1:
                                ...
                                printt_err(
                                    "- TRANSFER_FROM_FAILED         --> GASLIMIT too low. Raise it to GASLIMIT = 1000000 at least")
                                printt_err(
                                    "- TRANSFER_FROM_FAILED         --> Token is not approved for trade. LimitSwap will check approval right now.")
                                printt_err(
                                    "- INSUFFICIENT_OUTPUT_AMOUNT   --> SLIPPAGE too low")
                                printt_err(
                                    "- TRANSFER_FAILED              --> Trading is not enabled. Use WAIT_FOR_OPEN_TRADE parameter after reading wiki")
                                ...
                                check_approval(
                                    token, token['_IN_TOKEN'], token['_TOKEN_BALANCE'] * 1000000000000000000, 'txfail')

                                printt_debug(
                                    "3095 _FAILED_TRANSACTIONS:", token['_FAILED_TRANSACTIONS'])
                            else:
                                # transaction is a SUCCESS
                                printt_ok(
                                    "----------------------------------", write_to_log=True)
                                printt_ok(
                                    "SUCCESS : your sell Tx is confirmed    ", write_to_log=True)
                                ...
                                if token['XXX_SECONDS_COOLDOWN_AFTER_SELL_SUCCESS_TX'] != 0:
                                    printt_info("Bot will wait", token['XXX_SECONDS_COOLDOWN_AFTER_SELL_SUCCESS_TX'],
                                                "seconds after SELL, due to XXX_SECONDS_COOLDOWN_AFTER_SELL_SUCCESS_TX parameter", write_to_log=True)
                                    sleep(
                                        token['XXX_SECONDS_COOLDOWN_AFTER_SELL_SUCCESS_TX'])
                                ...
                                token['_SUCCESS_TRANSACTIONS'] += 1
                                printt_debug(
                                    "3900 _SUCCESS_TRANSACTIONS:", token['_SUCCESS_TRANSACTIONS'])
                                ...
                                token['_TOKEN_BALANCE'] = check_balance(
                                    token['ADDRESS'], token['SYMBOL'], display_quantity=True) / token['_CONTRACT_DECIMALS']
                                printt_ok(
                                    "----------------------------------", write_to_log=True)
                                ...
                                if token['_TOKEN_BALANCE'] < Decimal(token['MAXTOKENS']):
                                    token['_REACHED_MAX_TOKENS'] = False
                                    printt_info(
                                        "Your balance < MAXTOKENS for", token['SYMBOL'], "token --> BUY re-enabled", write_to_log=True)
                                else:
                                    token['_REACHED_MAX_TOKENS'] = True
                                    printt_info("You are still above MAXTOKENS for",
                                                token['SYMBOL'], "token --> BUY disabled", write_to_log=True)

                else:
                    if settings['VERBOSE_PRICING'] == 'true':
                        printt("Trading for token",
                               token['_PAIR_SYMBOL'], "is disabled")
...
def runLoop():
    while True:
        try:
            ...
            printt_err(
                "ERROR. Please go to /log folder and open your logs: you will find more details.")
            logging.exception(e)
            logger1.exception(e)
            sys.exit()
...
try:
    ...
    printt_err(
        "ERROR SETTINGS . Please go to /log folder and open your logs: you will find more details.")
    logging.exception(e)
    logger1.exception(e)
    sys.exit()
